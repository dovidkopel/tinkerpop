...
public abstract class AbstractLambdaTraversal<S, E> implements Traversal.Admin<S, E> {
    ...
    @Override
    public <T> Optional<T> metadata(final String key) {
        return null == this.bypassTraversal ? Optional.empty() : this.bypassTraversal.metadata(key);
    }

    @Override
    public <T> void metadata(final String key, final T value) {
        if (null != this.bypassTraversal)
            this.bypassTraversal.metadata(key, value);
    }
